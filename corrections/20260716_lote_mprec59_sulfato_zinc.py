...
tmpl = env['product.template'].search([('default_code', '=', 'MPREC59')])
producto = tmpl.product_variant_ids[0]
print("Producto:", tmpl.default_code, '-', tmpl.name)

factory = env['amunet.lot.factory'].search([('name', '=', '260822')], limit=1)
if not factory:
    factory = env['amunet.lot.factory'].create({'name': '260822'})
...
env.cr.commit()
print("Inventario: 50 g de REC59082201 en", ubicacion.complete_name)
