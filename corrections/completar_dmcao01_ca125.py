# Completa el producto terminado DMCAO01 (CA125) linea corta. Portable.
#  - Nombre corto "CA125" (en_US + es_MX)
#  - Secuencia de folio MO /CA125 (crea y liga)
#  - BoM (por pieza): SPHMC33 0.40cm, MPCAR33 1, STDSC01 1, STGOT04 1, MPBOL01 1
#    (reemplaza BoM 92: product_qty=70). Gotero = STGOT04 (25 ul).
#  - Ruta 8 pasos estandar (supervision/inspeccion)
#  - Presentaciones 5 y 10 pzas: MICAJ01 + STBPR01 + MIMAN01 (1 c/u; vial va en empaque)
# Autorizado por Fernando 2026-07-27.
Prod = env['product.product'].sudo()
t = env['product.template'].sudo().search([('default_code', '=', 'DMCAO01')], limit=1)
assert t, 'DMCAO01 no existe'
...
t.with_context(lang='en_US').write({'name': 'CA125'})
t.with_context(lang='es_MX').write({'name': 'CA125'})
print('Nombre:', t.with_context(lang='es_MX').name)

if not t.mo_sequence_id:
    seq = env['ir.sequence'].sudo().create({
        'name': 'Folio MO CA125', 'prefix': '%(month)s%(y)s/', 'suffix': '/CA125',
...
ops = [
    (5,  'Surtido de materiales - CA125',                        'Almacén Materia Prima',    False, False),
    (10, 'Serigrafiado de empaque primario - CA125',            'Acondicionado 2',          True,  True),
    (20, 'Corte de hojas maestras compradas - CA125',           'Laminado, Secado y Corte', True,  False),
    (30, 'Encartuchado de tiras - CA125',                       'Encartuchado',             True,  True),
    (50, 'Acondicionado 1 - CA125',                             'Acondicionado 1',          True,  False),
    (55, 'Sellado de Empaque primario - CA125',                 'Acondicionado 1',          True,  False),
    (60, 'Acondicionado 2 - CA125',                             'Acondicionado 2',          True,  False),
    (70, 'Resguardo de producto en espera de analisis - CA125', 'Almacen Temporal PT',      True,  False),
]
