...
tmpl = env['product.template'].search([('default_code', '=', 'MPREC57')])
producto = tmpl.product_variant_ids[0]
print("Producto:", tmpl.default_code, '-', tmpl.name)

factory = env['amunet.lot.factory'].search([('name', '=', '230422')], limit=1)
if not factory:
    factory = env['amunet.lot.factory'].create({'name': '230422'})
...
lote = env['stock.lot'].create({
    ...
    'expiration_date': '2027-04-01 00:00:00',
})
...
env.cr.commit()
print("Inventario: 5 g de REC57082201 en", ubicacion.complete_name)
