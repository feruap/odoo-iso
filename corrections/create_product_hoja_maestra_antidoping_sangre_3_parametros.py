"""
create_product_hoja_maestra_antidoping_sangre_3_parametros.py

Crea "Hoja Maestra Antidoping Sangre 3 Parámetros" (SPHMC76)
copiando todos los atributos e imagen del producto base con default_code='SPHMC75'.

Idempotente:
  - Si ya existe con SPHMC76 e imagen → no hace nada.
  - Si ya existe con SPHMC76 pero sin imagen → copia imagen del origen.
  - Si existe con código incorrecto SPHMC75 y nombre correcto → corrige código e imagen.
  - Si no existe → lo crea desde cero.

Uso: python3 create_product_hoja_maestra_antidoping_sangre_3_parametros.py DB USER PASSWORD HOST PORT
"""
import sys
import json
...
SOURCE_CODE = 'SPHMC75'
NEW_NAME    = 'Hoja Maestra Antidoping Sangre 3 Par\u00e1metros'
NEW_CODE    = 'SPHMC76'
...
# ── Case 1: SPHMC76 ya existe con imagen ──────────────────────────────────────
cur.execute("""
...
# ── Case 2: Existe con código incorrecto (SPHMC75 + nombre correcto) ───────────
cur.execute("""
    SELECT pt.id, pp.id AS pp_id
    FROM product_template pt
    JOIN product_product pp ON pp.product_tmpl_id = pt.id
    ...
    LIMIT 1
""", (SOURCE_CODE, f'%{NEW_NAME}%', '%Antidoping Sangre%3%'))
row = cur.fetchone()
