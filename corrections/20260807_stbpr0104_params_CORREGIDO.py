...
PRODUCTOS = {
    ...
    'STBPR02': {
        ...
        'mavi07_redir_ids': [],
        # Resultados de QC reales (TLDs) que aun apuntan a specs viejas a borrar:
        # preservarlos redirigiendolos a la canonica correcta (Neg->Neg, Pos->Pos).
        'tld_a_neg': [77998],   # spec vieja "Muestra negativa" -> canonica Neg 78214
        'tld_a_pos': [77999],   # spec vieja "Muestra positiva" -> canonica Pos nueva
        'rel07': 102,
...
for code, cfg in PRODUCTOS.items():
    ...
    if cfg['mavi07_pos']:
        env.cr.execute("""
            ...
            WHERE id = %s
        """, (cfg['mavi07_pos'],))
        pos_id = cfg['mavi07_pos']
        print(f"  MAVI-07 Muestra positiva (id={pos_id}): actualizada")
    else:
        env.cr.execute("""
            ...
            RETURNING id
        """, (cfg['rel07'],))
        pos_id = env.cr.fetchone()[0]
        print(f"  MAVI-07 Muestra positiva: insertada (id={pos_id}, rel_id={cfg['rel07']})")

    # c.2) Preservar resultados de QC reales: redirigir TLDs de specs viejas a
    #      la canonica correcta (Neg->Neg, Pos->Pos) ANTES de borrar esas specs.
    for tld_spec in cfg.get('tld_a_neg', []):
        env.cr.execute(
            "UPDATE amunet_quality_test_line_detail SET specification_config_id = %s "
            "WHERE specification_config_id = %s", (cfg['mavi07_neg'], tld_spec))
        if env.cr.rowcount:
            print(f"  TLD(s) de spec {tld_spec} → Neg {cfg['mavi07_neg']} ({env.cr.rowcount})")
    for tld_spec in cfg.get('tld_a_pos', []):
        env.cr.execute(
            "UPDATE amunet_quality_test_line_detail SET specification_config_id = %s "
            "WHERE specification_config_id = %s", (pos_id, tld_spec))
        if env.cr.rowcount:
            print(f"  TLD(s) de spec {tld_spec} → Pos {pos_id} ({env.cr.rowcount})")
