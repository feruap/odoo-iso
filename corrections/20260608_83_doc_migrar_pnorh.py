...
DOCS = {
    'PNORH-001': ('PNORH-001.docx', 'RH', 'CATÁLOGO DE FIRMAS'),
    'PNORH-002': ('PNORH-002.docx', 'RH', 'EVALUACIÓN DE PERSONAL'),
    'PNORH-003': ('PNORH-003.docx', 'RH', 'CAPACITACIÓN DE PERSONAL'),
    'PNORH-004': ('PNORH-004.docx', 'RH', 'RECLUTAMIENTO Y SELECCIÓN DE PERSONAL'),
    'PNORH-005': ('PNORH-005.docx', 'RH', 'ELABORACIÓN Y ACTUALIZACIÓN DEL ORGANIGRAMA'),
    'PNORH-006': ('PNORH-006.docx', 'RH', 'EXPEDIENTE DEL PERSONAL'),
}
...
def extract_all(zf):
    ...
    for tbl in root.findall('.//w:tbl', NS):
        ...
        if any(k in hdr for k in ('RESPONSABILIDAD','ROL','PUESTO')):
            for row in rows[1:]:
                cells = row.findall('w:tc', NS)
                if len(cells) >= 2:
                    rol  = cell_plain(cells[0])
                    desc = cell_plain(cells[1])
    ...
    for tbl in root.findall('.//w:tbl', NS):
        ...
        col_h = [''.join(t.text or '' for t in c.findall('.//w:t', NS)).upper()
                 for c in rows[0].findall('w:tc', NS)]
    ...
    for child in children:
        tag = child.tag.split('}')[-1]
        if tag == 'p':
            text = para_text(child); text_upper = text.upper()
            mf = SECTION_MAP.get(text_upper)
            if mf:
...
for codigo, (fname, area, titulo) in DOCS.items():
    ...
    for h in historial:
        state_h = 'vigente' if h['version'] == version else 'obsoleto'
        VerModel.create({
    ...
    print(f'OK {codigo} | v{version} | sust={sustituye} | fe={fe} | fv={fv} | {len(actividades)} acts | {len(responsabilidades)} resp | {len(terminos)} terms | {len(historial)} hist | secs:{len(secs)}')
