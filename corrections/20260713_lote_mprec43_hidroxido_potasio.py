...
tmpl = env['product.template'].search([('default_code', '=', 'MPREC43')])
producto = tmpl.product_variant_ids[0]
print("Producto:", tmpl.default_code, '-', tmpl.name)

factory = env['amunet.lot.factory'].search([('name', '=', '29.10.25')], limit=1)
if not factory:
    factory = env['amunet.lot.factory'].create({'name': '29.10.25'})
...
lote = env['stock.lot'].create({
    ...
    'expiration_date': '2030-11-01 00:00:00',
})
...
env.cr.commit()
print("Inventario: 250 g de REC43122501 en", ubicacion.complete_name)
