"""
Actualiza los códigos de parámetros de las 3 aguas (MPABD01, MPADE01, MPATR01):
...
aguas = {
    'MPABD01': {'conductividad_max': 3.0,  'conductividad_text': '≤3 µs/cm'},
...
for code, cfg in aguas.items():
    ...
    for rel in rels_agua:
        ...
        # Límites microbianos: 0–100 UFC/10 mL
        elif rel.parameter_id.code == 'MGA-0571':
            specs_lm = SpecCfg.search([('product_parameter_rel_id', '=', rel.id)])
            specs_lm.write({'min_value': 0, 'max_value': 100,
                            'acceptance_criteria': '≤100 UFC/10 mL o D.O. ≤ 0.6',
                            'specification_name': 'Límites microbianos'})
            print(f"{code} Límites microbianos: {len(specs_lm)} spec(s) → ≤100 UFC/10 mL")
...
env.cr.commit()
print("LISTO — parámetros de aguas y checks existentes actualizados.")
