...
doc = DocModel.search([('codigo', '=', 'PNOCC-009')], limit=1)
if not doc:
    print('PNOCC-009 no encontrado'); exit()

acts = ActModel.search([('documento_id', '=', doc.id)], order='sequence')
act2 = acts[1]
act2.write({'descripcion': new_desc})
env.cr.commit()
print(f'OK PNOCC-009 — actividad 2 actualizada con {img_count} imágenes')
