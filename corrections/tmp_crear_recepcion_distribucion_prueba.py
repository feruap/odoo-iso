# Crea una recepción de prueba con EQMIC01 (Micropipeta 5-50µl) — categoría Distribución/Equipo
wh = env['stock.warehouse'].search([('code', '=', 'AMP')], limit=1)
pt_in = env['stock.picking.type'].search([
    ('warehouse_id', '=', wh.id), ('code', '=', 'incoming')
], limit=1)

producto = env['product.product'].search([('default_code', '=', 'EQMIC01')], limit=1)
...
print(f"Producto: {producto.display_name} | Categoría: {producto.categ_id.complete_name}")
