...
class AmunetDocumentoSugerencia(models.Model):
    ...
    texto_original = fields.Html(
        ...
        help='Como esta el texto al momento de hacer la sugerencia.')
    texto_propuesto = fields.Html(
        string='Texto propuesto', sanitize=True, sanitize_tags=False, required=True,
        help='Como quedaria el texto si aceptan la sugerencia. Edita lo que cambia.')
    motivo = fields.Text(
        string='Motivo del cambio', required=True, tracking=True,
        help='Por que propones este cambio.')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            # Asegurar snapshot del texto original aunque venga vacio (default robusto)
            ...
            if r.documento_id.elabora_id:
                r.documento_id.activity_schedule(
                    ...
                    note=_(
                        ...
                        '<p>Abre el documento y ve a la pestana "Sugerencias" para '
        ...
        for r in self:
            ...
            if r.documento_id.elabora_id \
                ...
                raise UserError(_(
                    ...
                    'sugerencias. Si lo necesita un responsable, este puede hacerlo '
                    'desde el menu de gerencia.'
            ...
            # Aplicar el cambio al campo de la seccion
            field = SECCION_FIELD_MAP.get(r.seccion)
            if field:
                # Usar sudo para escapar bloqueo de "vigente" si aplicara
            ...
            # Avisar al revisor que la propuso
            if r.creado_por_id:
                r.documento_id.activity_schedule(
                    'mail.mail_activity_data_todo',
                    summary=_('Sugerencia aceptada en %s') % r.documento_id.codigo,
                    note=_(
                        '<p>%s acepto tu sugerencia en la seccion <b>%s</b> '
                        'del documento <b>%s</b>.</p>'
                    ) % (self.env.user.name,
                         dict(SECCION_SELECTION).get(r.seccion, r.seccion),
                         r.documento_id.codigo),
                    user_id=r.creado_por_id.id,
                )
            r.documento_id.message_post(
                body=_(
                    '<p><b>Sugerencia aceptada</b> por %s en la seccion <b>%s</b>. '
                    'El contenido se actualizo.</p>'
                ) % (self.env.user.name,
                     dict(SECCION_SELECTION).get(r.seccion, r.seccion)),
            )
    ...
    def action_rechazar(self):
        for r in self:
            ...
            if not (r.motivo_rechazo or '').strip():
                raise UserError(_(
                    'Indica el motivo del rechazo en el campo "Motivo del rechazo".'))
            if r.documento_id.elabora_id \
                    and r.documento_id.elabora_id.id != self.env.user.id \
                    and not self.env.user.has_group('amunet_documentos.group_documentos_manager'):
                raise UserError(_(
            ...
            if r.creado_por_id:
                r.documento_id.activity_schedule(
                    ...
                    note=_(
                        '<p>%s rechazo tu sugerencia en la seccion <b>%s</b> '
                        'del documento <b>%s</b>.</p>'
                        '<p><b>Motivo del rechazo:</b> %s</p>'
                    ) % (self.env.user.name,
                         dict(SECCION_SELECTION).get(r.seccion, r.seccion),
                         r.documento_id.codigo, r.motivo_rechazo),
                    user_id=r.creado_por_id.id,
                )
            r.documento_id.message_post(
                body=_(
                    '<p><b>Sugerencia rechazada</b> por %s. '
                    '<b>Motivo:</b> %s</p>'
