...
class MrpProduction(models.Model):
    ...
    def _amunet_baja_rechazada_firma(self):
        ...
        # Cerrar la MO por la via de baja (el flag salta el candado de QC aprobado).
        res = self.with_context(amunet_baja_rechazada=True).button_mark_done()
        # Si Odoo pide backorder (se produjo menos de lo planeado), cerrar sin backorder.
        if isinstance(res, dict) and res.get('res_model') == 'mrp.production.backorder':
            ctx = dict(res.get('context') or {}, amunet_baja_rechazada=True)
            wiz = self.env['mrp.production.backorder'].with_context(ctx).create({
                'mrp_production_ids': [(6, 0, self.ids)],
                'mrp_production_backorder_line_ids': [
                    (0, 0, {'mrp_production_id': self.id, 'to_backorder': False})],
            })
            wiz.action_close_mo()
        self.sudo().message_post(body=_(
