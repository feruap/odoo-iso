...
class StockPicking(models.Model):
    ...
    # ── Al llegar el combo a Existencias, generar el 2o ingreso (conversion) ─
    def _action_done(self):
        ...
        for picking in self:
            ...
            wh = picking.picking_type_id.warehouse_id \
                or self._amunet_combo_warehouse()
            lot_stock = wh.lot_stock_id
            # combos que acaban de aterrizar en Existencias (paso final de la
            # ruta de recepcion, sea 1, 2 o 3 pasos)
            combo_moves = picking.move_ids.filtered(
                lambda m: m.state == 'done'
                and m.product_id.product_tmpl_id.es_combo_compra
                and m.location_dest_id == lot_stock)
            if combo_moves:
                picking._amunet_create_combo_conversion(combo_moves, lot_stock)
        return res

    def _amunet_create_combo_conversion(self, combo_moves, lot_stock):
        self.ensure_one()
