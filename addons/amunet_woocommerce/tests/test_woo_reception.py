...
class TestWooReception(AmunetWooCommon):
    ...
    def test_release_gate_soft_dependency(self):
        """Sin el módulo de Calidad no hay concepto de liberación: no bloquea."""
        Lot = self.env['stock.lot']
        if 'amunet_lot_release_state' in Lot._fields:
            self.skipTest(
                'Con Calidad instalado el candado se prueba en '
                'test_release_gate_blocks_unreleased')
        self.assertFalse(self.Reception._release_gate_field_exists())
        # Debe permitir aceptar (no hay liberación que exigir).
        rec = self._make_reception(qty=5.0)
        self.assertEqual(rec.state, 'aceptada')

    def test_release_gate_blocks_unreleased(self):
        """Con Calidad instalado: no se acepta un lote no liberado."""
        Lot = self.env['stock.lot']
        if 'amunet_lot_release_state' not in Lot._fields:
            self.skipTest('Módulo de Calidad no instalado en esta BD de prueba')
        # Se usa un lote NUEVO, nunca liberado: el modulo de Calidad bloquea
        # modificar campos criticos de un lote ya liberado (exige reanalisis o
        # desviacion/CAPA), asi que no se puede "des-liberar" self.lot.
        ...
        with self.assertRaises(UserError):
            self._make_reception(qty=5.0, lot=pendiente)
        # Una vez que Calidad lo libera, el almacen ya puede recibirlo.
        pendiente.amunet_lot_release_state = 'released'
        rec = self._make_reception(qty=5.0, lot=pendiente)
        self.assertEqual(rec.state, 'aceptada')
        self.assertEqual(rec.lot_id, pendiente)
