...
MAPPING_POSITIVE = (
    ...
    '{"sample_type":"positive","result":"result_6","verdict":"invalid","message":"PRUEBA INVÁLIDA: Patrón #6 - Sin línea control visible"},'
    '{"sample_type":"positive","result":"result_7","verdict":"invalid","message":"PRUEBA INVÁLIDA: Patrón #7 - Sin línea control ni línea T"},'
    '{"sample_type":"positive","result":"na","verdict":"not_applicable","message":"Muestra Positiva: Control no disponible - N/A"}'
    ']}}'
)

MAPPING_NEGATIVE = (
    '{"fixed_sample_type":"negative","positions":[' + _NEG_BASE + '],'
    ...
    '{"sample_type":"negative","result":"result_6","verdict":"invalid","message":"PRUEBA INVÁLIDA: Patrón #6 - Sin línea control visible"},'
    '{"sample_type":"negative","result":"result_7","verdict":"invalid","message":"PRUEBA INVÁLIDA: Patrón #7 - Sin línea control ni línea T"},'
...
def migrate(cr, version):
    """Migración 3.15.0: MAVI-07 — patrones #6 y #7 (INVÁLIDA, sin línea control)."""

    # Actualizar spec_configs de Muestra positiva
    cr.execute("""
    ...
    # Actualizar spec_configs de Muestra negativa
    cr.execute("""
        UPDATE amunet_quality_parameter_specification_config sc
        SET text_phrase_mapping = %s, write_date = NOW()
        FROM amunet_quality_parameter_product_rel r
    ...
    # Actualizar text_phrase_mapping cacheado en checks en progreso/borrador
    cr.execute("""
        ...
        WHERE tld.test_line_id = tl.id
          ...
          AND qc.state IN ('in_progress','draft')
          AND pt.default_code = ANY(%s)
    """, (MAPPING_POSITIVE, list(SPHM_CODES)))
    pos_det = cr.rowcount
    ...
    _logger.info(
        "Migración 3.15.0 completa — spec_configs: pos=%d neg=%d | details: pos=%d neg=%d",
