...
# Destinatarios extra de la notificacion de cierre: se configuran por
# parametro del sistema (Ajustes > Tecnico > Parametros del sistema), clave
# 'amunet_aviso.notify_extra_uids' = lista de UIDs separada por comas. Asi no
# hay UIDs fijos en el codigo; la membresia de grupos se gestiona por interfaz.
...
class AmunetAvisoProduccion(models.Model):
    ...
    def _notificar_cierre(self, resultado):
        ...
        # + destinatarios extra configurables (parametro del sistema)
        extra = self.env['ir.config_parameter'].sudo().get_param(
            'amunet_aviso.notify_extra_uids', '')
        extra_uids = [int(x) for x in extra.split(',') if x.strip().isdigit()]
        if extra_uids:
            destinatarios |= self.env['res.users'].sudo().browse(extra_uids).exists()
        # + Calidad
