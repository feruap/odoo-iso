...
class AmunetMaterialRequest(models.Model):
    ...
    amunet_requiere_compra = fields.Boolean(
        string='Almacen no la surte completa',
        compute='_compute_amunet_requiere_compra',
        store=True,
        help='Alguna linea pide mas de lo que hay en el almacen. Que se haga '
             'con el faltante es tema de Compras, en su propio modulo: aqui '
             'solo se registra que el almacen no alcanza.',
    )
    ...
    def _check_can_submit(self):
        ...
        for line in self.line_ids:
            ...
            # queda marcada como pendiente. Pedir la compra es un tramite
            # aparte, en el modulo de compras.
