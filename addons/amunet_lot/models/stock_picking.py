...
class StockPicking(models.Model):
    ...
    transfer_approved = fields.Boolean(
        string='Aprobado',
        default=False,
        copy=False,
        tracking=True,
    )
    transfer_approved_by = fields.Many2one(
        'res.users',
        string='Aprobado por',
        readonly=True,
        copy=False,
    )
    transfer_approved_date = fields.Datetime(
        string='Fecha aprobación',
        readonly=True,
        copy=False,
    )
    transfer_done_by_operator = fields.Boolean(
        string='Realizado por almacenista',
        default=False,
        copy=False,
        tracking=True,
    )
    transfer_done_by = fields.Many2one(
        'res.users',
        string='Realizado por',
        readonly=True,
        copy=False,
    )
    transfer_done_date = fields.Datetime(
        string='Fecha realización',
        readonly=True,
        copy=False,
    )
    transfer_verified = fields.Boolean(
        string='Verificado',
        default=False,
        copy=False,
        tracking=True,
    )
    transfer_verified_by = fields.Many2one(
        'res.users',
        string='Verificado por',
        readonly=True,
        copy=False,
    )
    transfer_verified_date = fields.Datetime(
        ...
        for rec in self:
            ...
            try:
                ...
                if rec.scheduled_date:
                    dt = dt.replace(
                        hour=rec.scheduled_date.hour,
                        minute=rec.scheduled_date.minute,
                    )
    ...
    def action_confirm(self):
        # Para traslados internos: si el usuario agregó move_lines en borrador sin
        # move_id padre, crear los stock.move correspondientes antes de confirmar.
        for picking in self:
    ...
    def action_transfer_verify(self):
        """Supervisor verifica y dispara la validación real (mueve inventario)."""
        self.ensure_one()
