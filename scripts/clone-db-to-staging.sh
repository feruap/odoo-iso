...
CONTAINER_BACKUP="/tmp/restore_staging.sql"
...
docker exec "$PROD_CONTAINER" pg_dump --no-owner --no-privileges -U odoo "$PROD_DB" > "$BACKUP_FILE"
...
# Copiamos el archivo al contenedor para evitar restriccion de comandos backslash en stdin
docker cp "$BACKUP_FILE" "${STAGING_CONTAINER}:${CONTAINER_BACKUP}"
docker exec "$STAGING_CONTAINER" psql -U odoo -d "$STAGING_DB" -f "$CONTAINER_BACKUP"
